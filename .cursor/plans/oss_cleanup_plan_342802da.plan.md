...
overview: "Подготовить Metronix Core к open-source публикации через безопасную поэтапную очистку: убрать внутренние артефакты, секретные/продовые следы, демо-пайплайн и конфликтующие install-доки; затем собрать понятную публичную структуру документации с README, manual.md, install.md, connecting_to_agent.md, open-core boundary и agent integration guides."
todos:
...
- `connecting_to_agent.md` создается как отдельная инструкция: пользователь передает агенту prompt из `prompt.txt`, агент сам спрашивает недостающие параметры и подключает Metronix по MCP.
...
- AI/dev-team artifacts: `.claude/settings.local.json`, `src/metronix/**/.claude/**`; не публиковать root `CLAUDE.md` как есть, вместо этого вынести sanitized architecture/config content в public docs.
...
- Добавить startup validation для production/staging defaults в `src/metronix/core/config.py`: `secret_key="change-me-in-production"`, `auth_password="metronix"`, слабые DB/MCP defaults.
- Убрать inline weak secrets из `install/metronix-k8s.yaml` и `docker-compose.full.yml`; оставить placeholders или external secret pattern.
...
- Удалить `scripts/memgraph_syntax_test.py` или переписать под Neo4j; сейчас импортирует несуществующий `metronix.storage.memgraph`.
...
- Удалить/переписать stubs после проверки тестов: `src/metronix/api/routes/benchmarker.py`, unused `src/metronix/observability/tracer.py`, `src/metronix/observability/health.py`, `src/metronix/retrieval/graph_enrichment.py`, `src/metronix/retrieval/fallback.py`.
- Оставить на отдельные PR: `src/metronix/channels/`, `src/metronix/api/routes/chat.py`, `src/metronix/agent/sessions.py`, `src/metronix/api/routes/finops.py`, `src/metronix/skills/`, compatibility shims under `src/metronix/memory/freshness/`.

## Phase 4: Product Boundary Cleanup

Сделать Core понятным как OSS-продукт:

- Убрать UI-CC service/image/profile из public install artifacts: `install/docker-compose.yml`, `install/metronix-k8s.yaml`, старых install-доков (`INSTALL.md`, `docs/INSTALL.md`) и `installer/README.md`.
- Оставить `src/metronix/core/plugin.py` как нейтральный plugin extension example, но убрать private `metronix_enterprise` naming из public-facing docs или заменить на generic `acme_plugin`.
...
- `connecting_to_agent.md`: инструкция, что нужно передать приложенный prompt агенту/LLM. Prompt из `c:\Users\vasne\Downloads\prompt.txt` используется как источник, но перед публикацией его нужно привести к публичной форме: убрать Hermes-only формулировки из общего текста, убрать упоминание commercial-version workspaces, явно перечислить параметры `METRONIX_URL`, `METRONIX_MCP_API_KEY`, `AGENT_UUID`, `DEFAULT_WORKSPACE_ID`, описать headers `Authorization: Bearer ...` и `X-Agent-Id`, verification через `metronix_status` и memory tools.
- `docs/integrations/*.md`: конкретные инструкции для Cursor, Claude Desktop, Hermes, OpenWebUI, LibreChat, OpenClaw.
- `docs/guides/ingestion.md`: REST upload, connector sync, MCP `metronix_store`, supported file formats; без небезопасного server-local path endpoint.
- `docs/product/open-core-boundaries.md`: Core содержит 4 DB stack, SPLADE, embedding proxy, freshness worker, metronix-api, memory/RAG/MCP/OAI/REST surfaces, UI-KB reference; UI-CC/Control Center описан как отдельный коммерческий продукт вне этого repo.
...
- Запустить repo-wide scans: `rg` по `MTRNIX-`, `atlassian.net`, `Control Center`, `UI-CC`, `metronix_enterprise`, `.claude`, `drp-m.mtrnix.com`, `change-me-in-production`, `chmod 777`, live-token patterns.
