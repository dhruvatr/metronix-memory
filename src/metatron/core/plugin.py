"""Plugin system — discovers and manages metatron-core extensions.

Enterprise plugins register via Python entry points (group="metatron.plugins").
...
Usage — writing a plugin (e.g. metatron-enterprise):
...
from metatron.core.events import EventBus, EventHandlerCallable
from metatron.core.interfaces import AuthBackendInterface, PipelineHook

logger = structlog.get_logger()


@runtime_checkable
class MetatronPlugin(Protocol):
    """Interface every metatron plugin must satisfy.
...
class PluginManager:
    ...
    def register_event_handler(
        ...
        Use the event name constants from ``metatron.core.events``
...
def discover_plugins(manager: PluginManager) -> None:
    """Discover and load all installed metatron plugins via entry points.

    Scans the ``"metatron.plugins"`` entry point group. Each entry point
    must resolve to a class implementing ``MetatronPlugin``.
    ...
    """
    try:
        eps = importlib.metadata.entry_points(group="metatron.plugins")
    ...
    if manager.loaded_plugins:
        ...
        logger.debug("plugins.none_found", group="metatron.plugins")
